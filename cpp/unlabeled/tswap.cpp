...
TSWAP::TSWAP(Grid *graph,Config starts,Config goals)
    :Solver(G,starts,goals),assignment_mode(GoalAllocator::BOTTLENECK_LINEAR)
{
    solver_name=SOLVER_NAME;
}
...
bool TSWAP::deadlockDetectResolve(Agent *a, std::vector<Agent *> &occupied_now)
{
    ...
    while (true)
    {
        if (b->v_now == b->g || b->v_next != nullptr)
            break; // not deadlock
