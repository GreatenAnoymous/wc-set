...
void LibGA::halt(const std::string &msg)
{
    std::cout << "error@Halt: " << msg << std::endl;
    // std::exit(1);
}
...
void LibGA::Matching::updateByIncrementalFordFulkerson(FieldEdge const *e)
{
    ...
    if (mate[s] == NIL)
    { // new path must include s
        dfs(s);
    }
    else if (mate[g] == NIL)
    { // new path must include g
        dfs(g);
    }
    else
    { // search all
...
void LibGA::Matching::solveBySuccessiveShortestPath()
{
    ...
    auto compare = [&](DijkstraNode *v, DijkstraNode *u)
    { return v->d > u->d; };
    ...
    for (int _i = 0; _i < N; ++_i)
    {
        ...
        while (!OPEN.empty())
        {
            ...
            for (auto m : adj[n->v])
            {
                ...
                if (c < 0)
                    halt("invalid cost: " + std::to_string(c));
        ...
        {
            if (!CLOSE[v])
                halt("unknown place: " + std::to_string(v));
            potential[v] += dist[v];
        }
