...
HighLevelNode::HighLevelNode(Config _C, DistTable &D, HighLevelNode *_parent)
    : C(_C),
      parent(_parent),
      priorities(C.size(), 0),
      order(C.size(), 0),
...
Solution Planner::solve()
{
    ...
    // setup search queues
    std::stack<HighLevelNode *> OPEN;
    std::unordered_map<Config, HighLevelNode *, ConfigHasher> CLOSED;
    std::vector<Constraint *> GC; // garbage collection of constraints

    // insert initial HighLevelNode
    auto S = new HighLevelNode(ins->starts, D);
    OPEN.push(S);
    CLOSED[S->C] = S;
    ...
    while (!OPEN.empty() && !is_expired(deadline))
    {
        ...
        // insert new search HighLevelNode
        auto S_new = new HighLevelNode(C, D, S);
        OPEN.push(S_new);
...
bool Planner::get_new_config(HighLevelNode *S, Constraint *M)
{
    // setup cache
    for (auto a : A)
    {
...
bool Planner::funcPIBT(Agent *ai)
{
    ...
    for (size_t k = 0; k < K; ++k)
    {
        ...
        C_next[i][k] = u;
        if (MT != nullptr)
            tie_breakers[u->id] = get_random_float(); // set tie-breaker
    }
    C_next[i][K] = ai->v_now;
    ...
    // failed to secure HighLevelNode
