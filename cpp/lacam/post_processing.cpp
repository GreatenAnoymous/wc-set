...
bool LACAM::is_feasible_solution(const LACAM::Instance &ins, const LACAM::Solution &solution,
                          const int verbose)
{
    if (solution.empty())
        return true;
...
int LACAM::get_makespan(const LACAM::Solution &solution)
{
    if (solution.empty())
        return 0;
...
int LACAM::get_path_cost(const Solution &solution, int i)
{
    const auto makespan = solution.size();
    const auto g = solution.back()[i];
    auto c = makespan;
    while (c > 0 && solution[c - 1][i] == g)
        --c;
    return c;
}

int LACAM::get_sum_of_costs(const Solution &solution)
{
    if (solution.empty())
        return 0;
    int c = 0;
    ...
    for (size_t i = 0; i < N; ++i)
        c += get_path_cost(solution, i);
    return c;
}

int LACAM::get_sum_of_loss(const Solution &solution)
{
    if (solution.empty())
        return 0;
    int c = 0;
    ...
    for (size_t i = 0; i < N; ++i)
    {
        ...
        {
            if (solution[t - 1][i] != g || solution[t][i] != g)
                ++c;
        }
    }
    return c;
...
int LACAM::get_makespan_lower_bound(const Instance &ins, DistTable &dist_table)
{
    int c = 0;
    for (size_t i = 0; i < ins.N; ++i)
    {
...
int LACAM::get_sum_of_costs_lower_bound(const Instance &ins, DistTable &dist_table)
{
    int c = 0;
    for (size_t i = 0; i < ins.N; ++i)
    {
...
void LACAM::print_stats(const int verbose, const Instance &ins,
                 const Solution &solution, const double comp_time_ms)
{
    auto ceil = [](float x)
    { return std::ceil(x * 100) / 100; };
...
void LACAM::make_log(const Instance &ins, const Solution &solution,
