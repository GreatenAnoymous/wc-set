...
PIBT_COMPLETE::PIBT_COMPLETE(Grid*graph, Config starts,Config goals): Solver(graph,starts,goals), comp_solver_type(COMP_SOLVER_TYPE::PUSH_AND_SWAP){
    solver_name = SOLVER_NAME;
    comp_time_complement = 0;
}
...
void PIBT_COMPLETE::run()
{
    ...
    {
        if (pathDist(i) > LB_makespan)
            LB_makespan = pathDist(i);
    }

    // solve by PIBT
    Problem _P = Problem(P->getG(), P->getConfigStart(), P->getConfigGoal(),
    ...
    { // PIBT failed
        ...
        Problem _Q = Problem(P->getG(), solution.last(), P->getConfigGoal(),
