...
PushAndSwap::PushAndSwap(Grid *graph, Config starts, Config goals) : Solver(graph, starts, goals), flg_compress(true), disable_dist_init(false)
{
    solver_name = PushAndSwap::SOLVER_NAME;
}
                          ...
                          std::vector<int> &occupied_now)
{
    ...
    while (occupied_now[ideal_loc_s->id] != NIL)
    {
        ...
        //
        if(p.size()==1)
            halt("marked as failed");
        // _r tries to move p[1]
        // if(p[1]->id>occupied_now.size()){
        //     std::cout<<"overflow  "<<p[1]->id<<"  "<<occupied_now.size()<<std::endl;
        // }
